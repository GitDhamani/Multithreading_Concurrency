...
to zero. If it is zero, then it will unlock the mutex. and go to sleep because it knows it shouldn't be executing first now.
...
sent a notification to the condition variable to wake up and try again.

So the conditional variable in start2 wakes up and checks the state of correctStart and this time finds it to be true. So it re-acquires 
the lock and starts its task before releasing the lock on the way out.
